...
Router.register('/categories', function() {
  ...
  el.innerHTML = '<div class="toolbar">'
    ...
    + (Auth.isManager() ? '<button class="btn primary sm" onclick="openCategoryForm()">+ New Category</button>' : '')
    + '</div></div>'
  ...
  try {
    ...
    data.forEach(function(parent) {
      html += '<div class="card" style="margin-bottom:12px">'
        ...
        + (Auth.isManager() ? '<button class="btn sm" onclick="openCategoryForm(null,\'' + esc(parent.id) + '\')">+ Subcategory</button>' : '')
        + (Auth.isManager() ? '<button class="btn sm" onclick="openCategoryFormEdit(\'' + esc(parent.id) + '\')">Edit</button>' : '')
        + '</div></div>';
      ...
      if (parent.children && parent.children.length) {
        ...
        parent.children.forEach(function(child) {
          ...
          html += '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border)">'
            ...
            + (Auth.isManager() ? '<button class="icon-btn" onclick="openCategoryFormEdit(\'' + esc(child.id) + '\')" title="Edit">&#9998;</button>' : '')
            + (Auth.isManager() ? '<button class="icon-btn" onclick="deleteCategoryConfirm(\'' + esc(child.id) + '\',\'' + esc(child.name) + '\',' + (child.asset_count || 0) + ')" title="Delete">&#10005;</button>' : '')
            + '</div></div>';
        });
        html += '</div>';
      } else {
