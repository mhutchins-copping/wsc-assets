// Source of truth for the PowerShell enrolment script served at
// GET /enrol-script. Kept as a plain string (rather than a separate
// .ps1 file) because the worker runtime has no filesystem access --
// the bytes have to ship inside the JS bundle.
//
// Escaping notes for future edits:
//   - Every backtick in PowerShell (line continuation, literal `$env:`)
//     has to be escaped as \` inside the template literal.
//   - `${...}` is JS template-string interpolation; PowerShell doesn't
//     use that form (it uses `$(...)` for subexpressions), so there's
//     nothing to escape on that front today.

export const ENROL_SCRIPT = `# WSC Assets -- Device Enrolment Script
#
